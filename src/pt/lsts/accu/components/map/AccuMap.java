...
public class AccuMap extends MapView implements IMCSubscriber, AccuComponent {

	public static final String[] SUBSCRIBED_MSGS = { "Map", "EstimatedState",
			"PiccoloWaypoint" };
		...
		return new SimpleOnGestureListener() {
			...
			public void onLongPress(MotionEvent e) {
				...
				IMCMessage gotoMsg = IMCDefinition.getInstance().create("Goto",
						...
						"depth", 0, "speed", 1000, "speed_units", "RPM");

				IMCMessage maneuverMsg = IMCDefinition.getInstance()
						.create("ManeuverSpecification")
						.setValue("maneuver_id", "goto")
						.setValue("data", gotoMsg)
						.setValue("num_transitions", 1);

				IMCMessage planSpec = IMCDefinition.getInstance().create(
						"PlanSpecification", "plan_id", "simple_goto",
						"description", "simple goto plan", "start_man_id",
						"goto", "num_maneuvers", 1, "maneuvers", maneuverMsg);

				Accu.getInstance()
						.getIMCManager()
						.sendToActiveSys("PlanControl", "type", 0, "op",
								"LOAD", "plan_id", "simple_goto", "arg",
								planSpec);
				Accu.getInstance()
						.getIMCManager()
						.sendToActiveSys("PlanControl", "type", 0, "op",
								"START", "plan_id", "simple_goto");
			}
		};
	}
