...
public class JoystickPad extends View {
	...
	// All PAD dimensions should de density independent pixels (assuming a 240x320 screen)
	int PAD_VIEW_SIZE = 250;
	int PAD_CENTER_X = 125;
	int PAD_CENTER_Y = 125;
	float PAD_RADIUS = 125; // This must be float to avoid division by zero in translateTo methods
...
//		p.setStrokeWidth(5);
	...
	private float[] translateToAxis(float x, float y)
	{
		...
		res[1] = -((-(SCALE/2))+((SCALE/(padRadius*2))*(y-getTop()))); // INVERT Y AXIS
		return res;
	}
	...
	private float[] translateToView(float x, float y)
	{
		...
		res[1] = (-y + (SCALE/2))/(SCALE/(padRadius*2)); // INVERT Y AXIS
		return res;
	}
	...
	public void onDraw(Canvas canvas){
		...
		p.setStrokeWidth(5);
		canvas.drawCircle(padCenterX, padCenterY, padRadius, p);

		float coord[] = translateToView(axisX, axisY);
//		System.out.println(coord[0] + " " + coord[1]);
		// to see if the point is inside the circle
		boolean inCircle = Math.pow(coord[0]-padCenterX, 2)+Math.pow(coord[1]-padCenterY, 2)<=Math.pow(padRadius,2);
...
//		System.out.println("pad: "+event.getX()+ " "+event.getY());
		...
		if(event.getAction()==MotionEvent.ACTION_MOVE)
		{
			...
			float res[] = translateToAxis(event.getX()-pos[0], event.getY()-pos[1]);
			axisX = (int)res[0];
			axisY = (int)res[1];
			System.out.println("x : " + (event.getX()-getLeft()) + " y: " +  (event.getY()-getTop()) + "top: " + getTop() + "left: " + getLeft());
			System.out.println("rx : " + event.getRawX() + " ry: " +  event.getRawY());
		}
