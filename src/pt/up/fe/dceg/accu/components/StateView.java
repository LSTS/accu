...
public class StateView extends TableLayout 
implements IMCSubscriber
{
	public static final String[] SUBSCRIBED_MSGS = {"EstimatedState","Rpm","VehicleState"};
	...
	private void initialize() {
		LayoutInflater.from(context).inflate(R.layout.stateview_layout,this,true);
...
//			Log.i("teste",msg.toString());
			...
			tv.setText(""+MUtil.roundn(Math.toDegrees(msg.getFloat("phi")),2));
			
			tv = (TextView)findViewWithTag("pitch");
			tv.setText(""+MUtil.roundn(Math.toDegrees(msg.getFloat("theta")),2));
			
			tv = (TextView)findViewWithTag("yaw");
			tv.setText(""+MUtil.roundn(Math.toDegrees(msg.getFloat("psi")),2));	
		...
		if(msg.getAbbrev().equalsIgnoreCase("VehicleState"))
		{
			...
			tv.setText(""+msg.getString("last_error")); //FIXME add timestamp @ msg field "last_error_time"
			
			tv = (TextView)findViewWithTag("op_mode");
			tv.setText(""+msg.getString("op_mode"));
		}
		if(msg.getAbbrev().equalsIgnoreCase("Rpm"))
		{
